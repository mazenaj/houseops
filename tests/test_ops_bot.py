...
def test_ops_status_update_endpoint(client, mock_db):
    ...
    with patch("main.get_db", return_value=mock_db), \
         patch("main.verify_job_secret", return_value=True), \
         patch("app.ops_bot.get_ops_status_report", return_value="Test Report") as mock_report, \
         patch("app.ops_bot.send_ops_message") as mock_send:
         
        response = client.post(
            "/jobs/ops-status-update",
...
def test_global_exception_handler(client, mock_db):
    ...
    with patch("main.get_db", return_value=mock_db), \
         patch("main.get_prefix_token_count", side_effect=Exception("Database connection timed out")), \
...
def test_driver_arrival_nag_timeout_alert_normal_channel(mock_db):
    ...
    def mock_collection_routing(collection_name):
        ...
        if collection_name == "driver_schedule":
            ...
            mock_coll.where.return_value.where.return_value.where.return_value.stream.return_value = [mock_outing_doc]
    ...
    with patch("app.workflow.send_text_message"), \
         patch("app.workflow._notify_tier1_users") as mock_notify, \
         patch("app.ops_bot.send_ops_alert") as mock_ops_alert:
