...
* **Warning Trigger:** Implemented tracking within `run_agent_turn` in [app/vertex_client.py](file:///Users/terminal/houseops/app/vertex_client.py) to trigger a warning message if the cumulative token count (`cumulative_prompt + cumulative_candidates`) reaches 250,000 tokens during an operation.
* **State Pause (Firestore):** When the threshold is crossed, the execution loop is paused, and the complete execution state (conversation history payload serialized via `Content.to_dict()`, token counts, and round indexes) is saved to Firestore under a pending confirmation with the action `resume_paused_agent_turn`.
* **Resume/Allow Hook:** Modified [app/confirmation_gate.py](file:///Users/terminal/houseops/app/confirmation_gate.py) to intercept `resume_paused_agent_turn` confirmations. If the user replies with a confirmation/resume keyword ("yes", "y", "confirm", "ok", "continue", "resume"), the gate clears the pending flag and returns `proceed_to_gemini=True` with the `resumed_payload` passed back to [main.py](file:///Users/terminal/houseops/main.py).
* **Resumed Execution:** Updated `run_agent_turn` to accept a `resumed_state` dictionary. When provided, it reconstructs the conversation history using `Content.from_dict()` and resumes the tool round loop starting directly from the paused round index.
* **Testing:** Added unit test `test_run_agent_turn_warning_and_resume` in [tests/test_main.py](file:///Users/terminal/houseops/tests/test_main.py) to verify the 250,000-token warning pause, the state serialization, and the successful resumption of execution from the saved state.
